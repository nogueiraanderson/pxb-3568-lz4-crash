#!/usr/bin/env bash
# PXB-3568 Fix A: Test with LZ4 1.10.0
#
# Builds a PXC image with LZ4 1.10.0 replacing the system liblz4,
# then runs SST tests to verify no crash occurs.
set -euo pipefail
...
force_sst_on_node3() {
    local attempt="$1"
    log "=== Fix A: SST attempt $attempt/$SST_ATTEMPTS ==="
    ...
    while [ $elapsed -lt 120 ]; do
        ...
        if [ "$state" = "Synced" ]; then
            log "Node3 synced successfully via SST with LZ4 1.10.0"
            return 0
        fi
...
main() {
    log "=== PXB-3568 Fix A: LZ4 1.10.0 Upgrade ==="

    cleanup

    log "Building PXC image with LZ4 1.10.0..."
    ...
    log "LZ4 version in fixed image:"
    docker exec lz4-pxc-node1 bash -c \
        "ls -la /usr/lib64/liblz4.so*; ldd /usr/bin/xtrabackup | grep lz4" 2>&1 || true
    ...
    docker logs lz4-pxc-node1 > "$PROJECT_DIR/evidence/node1-fix-a.log" 2>&1 || true
    docker logs lz4-pxc-node3 > "$PROJECT_DIR/evidence/node3-fix-a.log" 2>&1 || true
    docker exec lz4-pxc-node1 bash -c "rpm -q lz4-libs; ls -la /usr/lib64/liblz4.so*" \
        > "$PROJECT_DIR/evidence/lz4-version-fix-a.txt" 2>&1 || true

    if [ "$all_passed" = true ]; then
        log ""
        log "=========================================="
        log "  FIX A PASSED: $SST_ATTEMPTS SST cycles with LZ4 1.10.0"
        log "=========================================="
    else
        log ""
        log "=========================================="
        log "  FIX A FAILED: Crash still occurs with LZ4 1.10.0"
        log "=========================================="
    fi
